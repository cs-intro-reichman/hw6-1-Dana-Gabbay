...
public class Runigram {
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		...
		for (int i=0; i<image.length; i++)
			for (int j=0; j<image[i].length; j++)
		{
			result [i][image[i].length-(j+1)]= image[i][j];
		}
		return result;
	}
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		for (int i=0; i<height; i++)
		{
			...
			{
				int newRow = i* image.length / height;
				int newCol = j* image[0].length / width;
	...
	public static Color blend(Color c1, Color c2, double alpha) {
		...
		r= (int)(c1.getRed()*alpha + c2.getRed()*beta);
		g = (int)(c1.getGreen()*alpha + c2.getGreen()*beta);
		b = (int)(c1.getBlue()*alpha + c2.getBlue()*beta);
